...
To install **cytofkit2** package, start R and run the following codes on R console:

```{r, eval=FALSE}
if (!requireNamespace("BiocManager", quietly = TRUE))
    install.packages("BiocManager")
BiocManager::install("JinmiaoChenLab/cytofkit2")
```

`Notes`: **cytofkit2** GUI is dependent on XQuartz windowing system (X Windows) on Mac (OS X > 10.7). Install XQuartz from http://xquartz.macosforge.org.
...
# Options for Using **cytofkit2** Package

**cytofkit2** provides three ways to employ the workforce of this package:

## Run with GUI 

The easiest way to use **cytofkit2** package is through the ShinyApp. The GUI provides all main options of **cytofkit2** on a visual interface. To launch the GUI, load the package and type the following command:
...
![cytofkit2 GUI](cytofkit2_GUI.png)
...
**Cytofkit2** provides a core function `cytofkit()` to drive the analysis pipeline of mass cytometry data. Users only need to define several key parameters to start their analysis automatically. One simple example of running cytofkit2 using the core function is like this:
...
You can customize the parameters for your own need, run `?cytofkit2` to get information of all the parameters for `cytofkit2`. As running with GUI, once the analysis is done, the results will be saved under `resultDir` automatically.
      

## Run with Commands (Step-by-Step) 

You can make use of the functions exported from cytofkit2 to make your analysis more flexible and fit your own need. Here we use a sample data for demo:
...
markers <- c("(Sm150)Di<GranzymeB>", "(Pt195)Di<Cisplatin>")
...
```{r}
## See documentation, this function uses the output of main cytofkit2 function as its input
?cytof_clusterMtrx
```
